...
- The server's container image now pulls from `eclipse-temurin` instead of
  Docker Hub's official `openjdk` image, which was deprecated and removed
  years ago (the old config's `latest` tag no longer resolves at all).
- Lombok is bumped to a JDK-21-compatible release, and the repo registers
  its own JDK 21 toolchain rather than using rules_java's newer default,
  since Lombok's javac-internals hooks don't support anything past JDK 21
  yet.
- The client's JS/TS dependency lock (`pnpm-lock.yaml`, which
  `aspect_rules_js` needs) is now a committed, static file built from
  `yarn.lock` rather than regenerated by Bazel on every build — the live
  regeneration path proved non-deterministic across otherwise-identical
  builds.
...
`bazel test //...` and `bazel build //...` are green in CI on this PR
(Bazel 9.2.0, run from a clean checkout with normal internet access) —
every Go, Java, Scala, and TypeScript target builds, and the container
image resolves and links. Along the way CI also surfaced a few
pre-existing issues this PR fixes as part of getting there: Lombok
1.18.12 and the Docker Hub `openjdk` base image were already broken
(both dropped from upstream years ago), independent of the Bazel
version.
...
- Run `bazel run @maven//:pin` once merged to regenerate a pinned
  ...
  forward).
- After editing `package.json`, refresh `yarn.lock` as usual, then run
  `pnpm import` to regenerate `pnpm-lock.yaml` from it and commit both —
